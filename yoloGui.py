...
# Function to perform detection (runs in a separate thread)
def detect_objects():
...
# Main Tkinter window
root = tk.Tk()
root.title("Object Detection with YOLOv8")
...
btn_select = ttk.Button(frame, text="Select File", command=lambda: select_image() if mode_var.get() == "Image" else select_video(), width=20)
...
class_dropdown.grid(row=1, column=1, padx=10, pady=10, sticky=tk.W)

# Detection button
btn_detect = ttk.Button(frame, text="Detect Objects", command=start_detection, width=20)
btn_detect.grid(row=2, column=0, padx=10, pady=10, sticky=tk.W)

# Save Image button (visible only for images)
btn_save_image = ttk.Button(frame, text="Save Image", command=save_image, width=20)
btn_save_image.grid(row=2, column=1, padx=10, pady=10, sticky=tk.W)

# Image display panel
panel = ttk.Label(frame)
panel.grid(row=3, column=0, columnspan=2, pady=10)
...
progress_bar.grid_forget()  # Initially hidden

# Status bar at the bottom
status_label = ttk.Label(frame, text="Ready", anchor="w")
status_label.grid(row=5, column=0, columnspan=2, pady=10, sticky=(tk.W, tk.E))

# Loading label for progress indication
loading_label = ttk.Label(frame, text="", anchor="w")
loading_label.grid(row=6, column=0, columnspan=2, pady=10)
...
# Toggle mode (image or video)
def toggle_mode(mode):
    global is_video
    is_video = (mode == "Video")
    class_dropdown.grid(row=1, column=1, padx=10, pady=10, sticky=tk.W)
    btn_save_image.grid(row=2, column=1, padx=10, pady=10, sticky=tk.W) if not is_video else btn_save_image.grid_forget()
